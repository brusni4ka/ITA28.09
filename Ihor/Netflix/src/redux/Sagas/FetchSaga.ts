...
import { IMoviesRequested, MoviesFailed, MoviesRecieved,ISelectedMovieRequested,selectedMovieRecieved,selectedMovieFailed } from '../Actions/FetchActions';
import IMovie from '../../Interfaces/IMovie';


export const fetchMoviesApi  =  async(sortBy: string, searchBy?: string, search?: string): Promise<IMovie[]>  => {
    let queryUrl = searchBy && search ? `https://reactjs-cdp.herokuapp.com/movies/?${searchBy === "title" ? `search=${search}` : `filter=${search}`}&searchBy=${searchBy}&sortBy=${sortBy}&sortOrder=desc&limit=10`
    : `https://reactjs-cdp.herokuapp.com/movies?sortBy=${sortBy}&sortOrder=desc&limit=10`;
...
export const fetchSelectedMovieSub = () => {
    return takeLatest(FetchActionsTypes.SELECTED_MOVIE_REQUESTED, requestSelectedMovieSaga)
}
